...
資料來源為 **Google Sheets（公開 CSV 格式）**，系統啟動時自動並行拉取所有已設定工作表的資料，以進度條彈窗呈現載入進度，完成後需通過四位數密碼驗證方可使用。
...
### 2. 密碼驗證
- 進入系統需輸入四位數密碼（預設：`1982`）
- 密碼儲存於 `localStorage`，可持久保留
- 儲存系統設定時同樣需要密碼二次驗證
...
- 系統密碼以**明文**儲存於 `localStorage`，屬輕量級存取控制
- Google Sheets 試算表需設為「**任何知道連結的人均可檢視**」（公開 CSV）
- 密碼預設值為 `1982`，建議首次使用後透過設定面板修改
...
| 版本 | 說明 |
|------|------|
| `v20250923.02` | 當前版本（含顏色閾值、個別列設定、暱稱編輯、CSV 下載） |
